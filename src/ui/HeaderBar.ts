...
function createDatasetSelector(): HTMLElement {
    ...
    // Permanent sentinel option that opens the file picker
    ...
    // Hidden file input — triggered when the user picks "Import TIF…"
    ...
    fileInput.addEventListener('change', () => {
        ...
        if (!file) {
            // User cancelled — revert select to previous dataset
            select.value = appState.state.currentDataset
            return
        }
    ...
    function syncImportedOption(state: { currentDataset: string; importedFileName: string | null }): void {
    ...
    syncImportedOption(appState.state)

    select.addEventListener('change', () => {
        if (select.value === '__import__') {
            // Revert visually while file picker is open
            select.value = appState.state.currentDataset
            fileInput.click()
            return
        }
    ...
    appState.subscribe((state) => syncImportedOption(state))
